...
public class Spy
{
    public string CollectGettersAndSetters(string className)
    {
        ...
        foreach (var getterMeth in methods.Where(x=>x.Name.StartsWith("get")))
        {
            sb.AppendLine($"{getterMeth.Name} will return {getterMeth.ReturnType}");
        }

        foreach (var getterMeth in methods.Where(x => x.Name.StartsWith("set")))
        {
            sb.AppendLine($"{getterMeth.Name} will set field of {getterMeth.GetParameters().First().ParameterType}");
        }
